...
export default function DriverCompleted() {
  const groupedTasks = mockCompletedTasks.reduce((acc, task) => {
  ...
  return (
    <AppLayout>
      ...
      <header className="bg-card border-b border-border pt-safe px-4 pb-4">
        <div className="pt-4">
          <h1 className="text-2xl font-bold text-foreground">Дууссан хүргэлтүүд</h1>
        ...
        <div className="mt-4 flex gap-4">
          <div className="flex-1 bg-success/10 rounded-xl p-3 text-center">
            <span className="text-2xl font-bold text-success">{mockCompletedTasks.length}</span>
                ...
                >
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="font-bold text-foreground">{task.storeName}</h3>
                        <p className="text-sm text-muted-foreground">{task.customerName}</p>
                      </div>
                    </div>
                    <div className="text-right">
                      <span className="font-bold text-success">+₮{task.reward.toLocaleString()}</span>
                  ...
                  <div className="mt-2 text-xs text-muted-foreground">
                    {task.completedAt.toLocaleTimeString('mn-MN', { hour: '2-digit', minute: '2-digit' })}
                  </div>
                </motion.div>
              ))}
            </div>
